...
void GameTab::RemoveAction() {
	if (StackMill3DS::App->Down & KEY_TOUCH) {
		...
		for (int8_t Idx = 0; Idx < 24; Idx++) {
			if (Common::Touching(StackMill3DS::App->T, this->Fields[Idx])) {
	...
	if (StackMill3DS::App->Down & KEY_A) {
		...
		/* Wir können den Stein entfernen. */
		if (CanContinue) {
			this->Remove();
...
void GameTab::PlayerTurn() {
	...
	if (StackMill3DS::App->Down & KEY_TOUCH) {
		if (Common::Touching(StackMill3DS::App->T, this->Help)) {
			std::unique_ptr<Rules> Ovl = std::make_unique<Rules>();
			Ovl->Handler();
			return;
		};
...
void GameTab::AITurn() {
	...
	/* Do the turn. */
	std::pair<int8_t, int8_t> Turn = StackMill3DS::App->Core->AIRandomPlay();
	bool Status = false;
	...
	if (Status) {
		...
		switch(PStatus) {
			...
			case StackMill::PlayStatus::Match: {
				...
				this->Selection = this->Preview[rand() % (int8_t)this->Preview.size()];
				this->Remove();
...
void GameTab::PopupPrompt() {
	...
	while(aptMainLoop() && !Done) {
		...
		if (SwipedIn && !EndIt) {
			...
			if (Down & KEY_TOUCH) {
				if (Common::Touching(T, this->Prompt[0])) State = false, EndIt = true; // Exit.
				else if (Common::Touching(T, this->Prompt[1])) State = true, EndIt = true; // New Game.
			};
		};
